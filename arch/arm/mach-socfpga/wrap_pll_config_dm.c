...
const struct cm_config * const cm_get_default_config(void)
{
	struct cm_config *cm_handoff_cfg = (struct cm_config *)
		(HANDOFF_CLOCK + HANDOFF_OFFSET_DATA);
	u32 *conversion = (u32 *)cm_handoff_cfg;
	u32 i;
	u32 handoff_clk = readl(HANDOFF_CLOCK);

	if (swab32(handoff_clk) == HANDOFF_MAGIC_CLOCK) {
		writel(swab32(handoff_clk), HANDOFF_CLOCK);
		for (i = 0; i < (sizeof(*cm_handoff_cfg) / sizeof(u32)); i++)
			conversion[i] = swab32(conversion[i]);
		return cm_handoff_cfg;
	} else if (handoff_clk == HANDOFF_MAGIC_CLOCK) {
		return cm_handoff_cfg;
	}
