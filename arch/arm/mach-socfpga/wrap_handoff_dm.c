...
int get_handoff_size(void *handoff_address, enum endianness endian)
{
	...
	if (endian == little_endian) {
		handoff_size = (readl(handoff_address + HANDOFF_OFFSET_LENGTH) -
				HANDOFF_OFFSET_DATA) /
				sizeof(u32);
	} else if (endian == big_endian) {
		handoff_size = swab32(readl(handoff_address +
					    HANDOFF_OFFSET_LENGTH));
		handoff_size = (handoff_size - HANDOFF_OFFSET_DATA) /
				sizeof(u32);
	} else {
		return -EINVAL;
	}
		 ...
		 enum endianness big_endian)
{
	...
	if (big_endian) {
		if (swab32(readl(HANDOFF_BASE)) == HANDOFF_MAGIC_BOOT) {
			...
			debug("%s: Cannot find HANDOFF_MAGIC_BOOT ", __func__);
			debug("at addr  0x%p\n", (u32 *)handoff_address);
			return -EPERM;
		}
	} else {
		temp = readl(handoff_address);
		if (temp == DDR_HANDOFF_UMCTL2_MAGIC) {
			debug("%s: umctl2 handoff data =\n{\n",
			      __func__);
		} else if (temp == DDR_HANDOFF_PHY_MAGIC) {
			debug("%s: PHY handoff data =\n{\n",
			      __func__);
		} else if (temp == DDR_HANDOFF_PHY_INIT_ENGINE_MAGIC) {
		...
		if (temp == DDR_HANDOFF_UMCTL2_MAGIC ||
		    temp == DDR_HANDOFF_PHY_MAGIC ||
		    temp == DDR_HANDOFF_PHY_INIT_ENGINE_MAGIC) {
			/* Using handoff from Quartus tools if exists */
			for (i = 0; i < table_len; i++) {
				*table_x32 = readl(handoff_address +
						HANDOFF_OFFSET_DATA + (i * 4));
