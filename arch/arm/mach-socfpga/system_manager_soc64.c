...
{
	u32 len, i;
	u32 len_mux = get_handoff_size((void *)HANDOFF_MUX, big_endian);
	u32 len_ioctl = get_handoff_size((void *)HANDOFF_IOCTL, big_endian);
	u32 len_fpga = get_handoff_size((void *)HANDOFF_FPGA, big_endian);
	u32 len_delay = get_handoff_size((void *)HANODFF_DELAY, big_endian);
	...
	/* setup the pin sel */
	len = (len_mux < HANDOFF_MUX_LEN) ? len_mux : HANDOFF_MUX_LEN;
	handoff_read((void *)HANDOFF_MUX, handoff_table, len, big_endian);
	for (i = 0; i < len; i = i + 2) {
	...
	/* setup the pin ctrl */
	len = (len_ioctl < HANDOFF_IOCTL_LEN) ? len_ioctl : HANDOFF_IOCTL_LEN;
	handoff_read((void *)HANDOFF_IOCTL, handoff_table, len, big_endian);
	for (i = 0; i < len; i = i + 2) {
		writel(handoff_table[i + 1],
		       handoff_table[i] +
		       (u8 *)socfpga_get_sysmgr_addr() +
	...
	/* setup the fpga use */
	len = (len_fpga < HANDOFF_FPGA_LEN) ? len_fpga : HANDOFF_FPGA_LEN;
	handoff_read((void *)HANDOFF_FPGA, handoff_table, len, big_endian);
	for (i = 0; i < len; i = i + 2) {
		writel(handoff_table[i + 1],
		       handoff_table[i] +
		       (u8 *)socfpga_get_sysmgr_addr() +
	...
	/* setup the IO delay */
	len = (len_delay < HANODFF_DELAY_LEN) ? len_delay : HANODFF_DELAY_LEN;
	handoff_read((void *)HANODFF_DELAY, handoff_table, len, big_endian);
	for (i = 0; i < len; i = i + 2) {
		writel(handoff_table[i + 1],
		       handoff_table[i] +
		       (u8 *)socfpga_get_sysmgr_addr() +
